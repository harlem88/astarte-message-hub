...
use astarte_message_hub::AstarteMessageHubOptions;
use log::info;

pub fn read_options(
    override_config_file_path: Option<String>,
) -> Result<AstarteMessageHubOptions, AstarteMessageHubError> {
    let paths = [
        "astarte-message-hub-config.toml",
        "/etc/astarte-message-hub/config.toml",
    ]
    .iter()
    .map(|f| f.to_string());
    ...
    if let Some(path) = paths.into_iter().next() {
        ...
        let config = toml::from_str::<AstarteMessageHubOptions>(&config)?;
